...
KP.Assets = class Assets {
  ...
  drawShop(ctx,x,y,w,h){
    ...
    for(let i=0;i<5;i++){
      ctx.fillStyle=i%2?'rgba(255,210,28,.22)':'rgba(0,0,0,.15)';
      ctx.fillRect(x-16+i*(w+32)/5,y-30,(w+32)/5,30);
    }
    ctx.fillStyle='rgba(0,0,0,.3)'; ctx.fillRect(x-16,y-2,w+32,4);
    // Sign text
    ctx.fillStyle='#ffd21c'; ctx.font='bold 11px Arial'; ctx.textAlign='center';
    ctx.fillText('СНАБЖЕНЕЦ',cx,y-12); ctx.textAlign='left';
    // Red star on awning
    ctx.save(); ctx.fillStyle='#ffd21c'; ctx.translate(cx,y-22);
    ctx.beginPath();
    for(let i=0;i<5;i++){ ctx.rotate(Math.PI*2/5); ctx.lineTo(0,-7); ctx.lineTo(2.1,-2.8); }
    ctx.closePath(); ctx.fill(); ctx.restore();
    // Figure behind counter (head + shoulders silhouette)
    ...
    // Items on counter
    ctx.fillStyle='#c89000'; ctx.beginPath(); ctx.arc(x+10,y+h-26,5,0,Math.PI*2); ctx.fill();
    ctx.fillStyle='#444'; ctx.fillRect(cx-6,y+h-30,12,10);
    ctx.fillStyle='#ffd21c'; ctx.font='10px Arial'; ctx.fillText('★',cx-4,y+h-23);
    // Interaction glow
    ctx.strokeStyle='rgba(255,210,28,.25)'; ctx.lineWidth=1;
    ctx.strokeRect(x-14,y-30,w+28,h+30);
  }

  // --- Kamikaze (красно-оранжевый смертник) ---
  drawKamikaze(ctx,e){
    const x=e.x, y=e.y;
    ctx.save();
    if(e.facing<0){ ctx.translate(x+e.w,y); ctx.scale(-1,1); ctx.translate(-x,-y); }
    const pulse=e.explodeArmed&&e.hp<e.maxHp*.4?(0.6+Math.sin(Date.now()/60)*0.4):1;
    // Body
    ctx.fillStyle=e.hurt?'#fff5c9':'#8b1a00'; ctx.fillRect(x+6,y+18,28,30);
    // Red stripes on chest (X marker)
    ...
    if(pulse<1){ ctx.fillStyle=`rgba(255,80,0,${1-pulse})`; ctx.beginPath(); ctx.arc(x+14,y+4,3+4*(1-pulse),0,Math.PI*2); ctx.fill(); }
    ctx.restore();
  }

  // --- Shielder (тяжёлый щитоносец) ---
  drawShielder(ctx,e){
    const x=e.x, y=e.y;
    ctx.save();
    if(e.facing<0){ ctx.translate(x+e.w,y); ctx.scale(-1,1); ctx.translate(-x,-y); }
    ...
    ctx.fillStyle='#1a1a1c'; ctx.fillRect(x+14,y+8,18,5); // visor slit
    ctx.fillStyle='rgba(255,80,0,.7)'; ctx.fillRect(x+15,y+9,16,3); // glow visor
    // Front shield — large rectangular
  ...
  drawSniper(ctx,e){
    ...
    ctx.fillStyle='#4a6030'; ctx.fillRect(x+6,y+0,24,8); // ghillie hood
    // Eyes
    ctx.fillStyle='#1a1a0a'; ctx.fillRect(x+12,y+8,5,4); ctx.fillRect(x+22,y+8,5,4);
    // Long sniper rifle
    ctx.fillStyle='#1a1a1a'; ctx.fillRect(x+22,y+24,52,5);
    ctx.fillStyle='#333'; ctx.fillRect(x+58,y+22,10,9); // scope
    ctx.fillStyle='rgba(255,220,0,.5)'; ctx.fillRect(x+60,y+24,6,5);
    ctx.fillStyle='#111'; ctx.fillRect(x+22,y+28,20,3); // magazine
    // Feet
